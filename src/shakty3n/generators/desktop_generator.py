    ...
    def _generate_electron_project(self, description: str, requirements: Dict) -> Dict:
        ...
        package_data = {
            ...
            "scripts": {
                ...
                "dist:mac": "electron-builder --mac --arch universal"
            },
            "keywords": [],
            "author": "",
            "license": "MIT",
            "build": {
                "appId": self._build_app_id(description),
                "mac": {
                    "target": [
                        {
                            "target": "dmg",
        ...
        self.create_file("package.json", json.dumps(package_data, indent=2))
...
```bash
npm run dist:mac
# Output: dist/*.dmg (Intel & Apple Silicon)
```
...
Replace "YourAppName" with your app's name in the commands below.

```bash
pip install pyinstaller
pyinstaller --windowed --name "YourAppName" main.py

# Optional: wrap the app into a DMG installer (verify the app bundle path from PyInstaller output)
APP_PATH="dist/YourAppName.app"  # adjust if PyInstaller outputs elsewhere
hdiutil create -volname "YourAppName" -srcfolder "$APP_PATH" -ov -format UDZO YourAppName.dmg
```
...
"""
    ...
    def _build_app_id(self, description: str) -> str:
        """Create a deterministic appId from the project description"""
        text = description or ""
