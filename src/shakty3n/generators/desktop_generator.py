...
```bash
npm run dist:mac
# Output: dist/mac/*.dmg (Intel & Apple Silicon)
```
...
pyinstaller --windowed --name "Desktop Application" main.py

# Optional: wrap the app into a DMG installer
hdiutil create -volname "DesktopApp" -srcfolder \"dist/Desktop Application.app\" -ov -format UDZO DesktopApp.dmg
```
